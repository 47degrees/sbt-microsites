      ...
      div(
        ...
        ul(id := "sidebar",
           ...
           li(cls := "sidebar-brand",
              a(href := "{{ site.baseurl }}/",
                cls := "brand",
                div(cls := "brand-wrapper",
                    style := "background:url('{{site.baseurl}}/img/sidebar_brand.png') no-repeat",
                    span(config.name)))),
           ...
           li(
             a(href := "{{ site.baseurl }}{{x.url}}",
               ...
               "{{x.title}}")),
           "{% endif %} {% endfor %}")),
      pageContentWrapper(text))
  }

  def pageContentWrapper(text: String): TypedTag[String] = {
                ...
                div(cls := "row",
                    div(cls := "col-lg-12",
                        div(cls := "action-menu pull-left clearfix",
                            a(href := "#menu-toggle",
                              id := "menu-toggle",
                              i(cls := "fa fa-bars", aria.hidden := "true"))),
                        pullRight(text))))),
        div(id := "content",
            data("github-owner") := config.githubOwner,
            data("github-repo") := config.githubRepo,
            div(cls := "content-wrapper", section("{{ content }}"))))
  }

  def pullRight(text: String): TypedTag[String] = {
    ul(cls := "pull-right",
       li(cls := "hidden-xs",
