...
- `micrositeLayoutDirectory`: you can also introduce custom html layouts in the generated microsite through the `micrositeLayoutDirectory` setting, by using the same method. The layout files in that folder will be automatically copied and imported by the plugin in your microsite. The default value is `(resourceDirectory in Compile).value / "microsite" / "layout"` but you can override it like this:

```
micrositeLayoutDirectory := (resourceDirectory in Compile).value / "site" / "layout"
```
